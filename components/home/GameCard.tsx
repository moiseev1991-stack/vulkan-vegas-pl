...
export default function GameCard({ game, size = 'md', index = 0 }: GameCardProps) {
  ...
  return (
    <div className="group cursor-pointer">
      <div
        className="slot-card-glow relative rounded-xl overflow-hidden"
        style={{ animationDelay: glowDelay }}
      >
        {/* Image area */}
        <div className={`relative ${isSmall ? 'w-[120px] h-[120px]' : 'aspect-square w-full'} bg-[#130010]`}>

          {/* Diagonal shine sweep (idle) */}
          <div className="slot-card-shine" style={{ animationDelay: shineDelay }} />

          {/* Placeholder bg */}
          <div className="absolute inset-0 bg-gradient-to-br from-[#260015] to-[#130010] flex items-center justify-center">
        ...
        {game.tags.length > 0 && (
          <div className="absolute top-1.5 left-1.5 flex flex-col gap-0.5 z-20">
            {game.tags.includes('hot') && (
              <span className="text-white text-[9px] font-black px-1.5 py-0.5 rounded-sm" style={{ background: '#C4001C' }}>HOT</span>
            )}
            {game.tags.includes('network-promo') && (
              <span className="text-white text-[9px] font-black px-1.5 py-0.5 rounded-sm" style={{ background: '#C45A00' }}>NETWORK PROMO</span>
            )}
