...
export default function Header() {
  ...
  return (
    <header
      ...
      style={{ height: '56px', background: '#0A000B', borderBottom: '1px solid #2E001A' }}
    >
      <div className="container mx-auto px-4 flex items-center justify-between h-full">
        {/* Logo */}
        <Link href="/" className="flex items-center shrink-0">
          <Image src="/images/logo-official.png" alt="Vulkan Vegas" width={110} height={44} priority />
        </Link>
          ...
          {navLinks.map((link) =>
            link.hasDropdown ? (
              ...
              <div key={link.label} className="relative group">
                <Link
                  href={link.href}
                  className="relative flex items-center gap-1 px-3 py-2 rounded-md transition-colors whitespace-nowrap text-[13px] font-medium hover:text-white hover:bg-white/5"
                  style={{ color: isActive(link.href) ? '#fff' : '#DDDDDD' }}
                >
                  {link.label}
                ...
                <div
                  ...
                  style={{ background: '#130010', border: '1px solid #2E001A', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}
                >
              ...
              <Link
                key={link.label}
                href={link.href}
                className={`relative flex items-center gap-1 px-3 py-2 rounded-md transition-colors whitespace-nowrap text-[13px] font-medium
                  ${link.isSpecial
                    ? 'text-gold hover:text-gold/80'
                    : 'hover:text-white hover:bg-white/5'
                  }`}
                style={{ color: link.isSpecial ? undefined : isActive(link.href) ? '#fff' : '#DDDDDD' }}
              >
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          <Link
            ...
            style={{ height: '32px', display: 'flex', alignItems: 'center', background: '#FFC500' }}
          >
            ZAREJESTRUJ SIĘ
          </Link>
        </div>
      ...
      {/* Mobile drawer */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 top-14 bg-[#0A000B] z-40 overflow-y-auto">
          <div className="flex flex-col p-4 gap-1">
            ...
            <div className="flex flex-col gap-2 mt-4 px-4">
              ...
              <Link href="/go/" className="w-full py-3 text-center rounded-lg text-black text-sm font-bold" style={{ background: '#FFC500' }}>
                ZAREJESTRUJ SIĘ
              </Link>
            </div>
          </div>
